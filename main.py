...
# Estilo Dark e Ajustes de Layout Antigo + Trava de Toque no Gráfico
st.markdown("""
    <style>
    ...
    /* ESTA LINHA TRAVA O GRÁFICO PARA NÃO ATRAPALHAR A ROLAGEM DO CELULAR */
...
with st.expander("➕ REGISTRAR NOVO TURNO", expanded=True):
    with st.form("form_mobile", clear_on_submit=True):
        ...
        km = st.number_input("KM", min_value=0.0, step=0.1, format="%.1f") 
...
if not df.empty:
    # Gráfico de Linhas (Bolsa de Valores) - Estilo Antigo com Trava Estática
    fig = go.Figure()
    ...
    fig.update_layout(
        ...
        # ESSAS TRÊS LINHAS TRAVAM O GRÁFICO (SEM ZOOM, SEM ARRASTAR)
        dragmode=False,
        hovermode=False,
    ...
    # O comando 'staticPlot': True deixa o gráfico paralisado como uma foto
    ...
    m2.metric("Fundo Manutenção", f"R$ {manutencao_acumulada:,.2f}")
