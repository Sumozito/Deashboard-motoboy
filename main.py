...
# 1. CONFIGURAÇÃO DA PÁGINA (LIMPA E SEM ADM)
st.set_page_config(
...
# 2. CSS PARA OCULTAR BARRAS E MELHORAR O TOQUE NO CELULAR
st.markdown("""
    <style>
    ...
    /* Impede que o gráfico "roube" o toque de rolagem da página */
    .stPlotlyChart {
        pointer-events: none;
    }
...
if not df.empty:
    ...
    fig.update_layout(
        ...
        hovermode=False, # DESATIVA O HOVER (BALÃOZINHO)
        dragmode=False,  # DESATIVA O ARRASTAR (ZOOM)
        paper_bgcolor='rgba(0,0,0,0)',
        plot_bgcolor='rgba(0,0,0,0)',
        showlegend=False,
        margin=dict(l=0, r=0, t=10, b=0),
        xaxis=dict(showgrid=False, fixedrange=True), # TRAVA O EIXO X
        yaxis=dict(showgrid=True, gridcolor='#30363d', fixedrange=True) # TRAVA O EIXO Y
    )

    # config={'staticPlot': True} deixa o gráfico como uma imagem morta (paralisada)
